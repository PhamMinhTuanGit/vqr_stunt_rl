...
def cos_vertical(v: ca.MX) -> ca.MX:
    """Cosine of the angle between vector v and the world -z axis."""
    return -v[2] / (ca.norm_2(v) + EPS)
...
def measured(p: dict[str, Any], nominal_front_length: dict[str, float]) -> dict[str, Any]:
    ...
    m: dict[str, Any] = {
        ...
        "rear_thigh_vertical_deg": {},
        "rear_shank_vertical_deg": {},
    }
    for c in REAR:
        ...
        m["rear_thigh_vertical_deg"][c] = math.degrees(math.acos(np.clip(
            -thigh[2] / np.linalg.norm(thigh), -1, 1)))
        m["rear_shank_vertical_deg"][c] = math.degrees(math.acos(np.clip(
            -shank[2] / np.linalg.norm(shank), -1, 1)))
    for c in FRONT:
...
def stages(args: argparse.Namespace) -> list[dict[str, float | str]]:
    ...
    return [
        ...
        dict(name="B", pitch_min=-80, pitch_max=-55, vertical=25,
             straight=max(45, args.rear_straight_deg), front_ratio=0.93,
             clearance=0.035, line=0.010, midpoint=0.03),
...
def build_solver(geometry: RearPoseGeometry, cfg: dict[str, Any],
    ...
    # Wheeled-biped rear stance: each link (thigh, shank) near world
    # vertical, so the leg descends hip -> knee -> wheel with only the
    # unavoidable knee bend instead of a collapsed V.
    thigh_vertical = {
        c: cos_vertical(knees[c] - hips[c]) for c in REAR
    }
    shank_vertical = {
        c: cos_vertical(wheels[c] - knees[c]) for c in REAR
    }
    fold = {
    ...
    objective = (
        ...
        + 100.0 * sum((1 - t) ** 2 for t in (*thigh_vertical.values(),
                                             *shank_vertical.values()))
    ...
    constraints = ca.vertcat(
        ...
        straight["HL"], straight["HR"],
        thigh_vertical["HL"], thigh_vertical["HR"],
        shank_vertical["HL"], shank_vertical["HR"],
        fold["FL"], fold["FR"],
    )
    ...
    return ca.nlpsol("rear_axle_pose", "ipopt",
                     {"x": x, "f": objective, "g": constraints}, {
                         ...
                         # Monotone mu stalls against the knee bound (the
                         # straightness target may be unreachable; the bend
                         # floors at the URDF knee limit). Adaptive barrier
                         # converges in tens of iterations instead of 1200.
...
def constraint_bounds(stage: dict[str, Any], segment_margin: float) -> tuple[list[float], list[float]]:
    ...
    tv = math.cos(math.radians(stage["vertical"]))
    return (
        [0, 0, stage["clearance"], stage["clearance"],
         -stage["line"], -stage["midpoint"], -stage["midpoint"],
         segment_margin, segment_margin, c, c, tv, tv, tv, tv, 0, 0],
        [0, 0, np.inf, np.inf, stage["line"], stage["midpoint"],
         stage["midpoint"], np.inf, np.inf, 1, 1, 1, 1, 1, 1,
...
def check(geometry: RearPoseGeometry, cfg: dict[str, Any], x: np.ndarray,
    ...
    for c in REAR:
        ...
        if m["rear_thigh_vertical_deg"][c] > stage["vertical"] + 0.02:
            raise RuntimeError(f"{c} rear thigh is not vertical enough")
        if m["rear_shank_vertical_deg"][c] > stage["vertical"] + 0.02:
            raise RuntimeError(f"{c} rear shank is not vertical enough")
    for c in FRONT:
...
def main() -> int:
    ...
    for c in REAR:
        print(f"{c}: thigh/shank = {m['rear_thigh_shank_angle_deg'][c]:.2f} deg, "
              f"thigh vertical = {m['rear_thigh_vertical_deg'][c]:.2f} deg, "
              f"shank vertical = {m['rear_shank_vertical_deg'][c]:.2f} deg")
    for c in FRONT:
